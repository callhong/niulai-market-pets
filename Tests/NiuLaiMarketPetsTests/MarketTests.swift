...
struct MarketTestRunner {
    static func main() async {
        ...
        run("market target catalog and migration") {
            ...
            expect(MarketTarget.tonghuashun(code: "123456")?.name == "同花顺（123456）")
        ...
        run("notification policy boundary deduplication") {
            ...
            expect(!a && b && !c && d && !e && !f)
            let csiQuote = Quote(symbol: MarketTarget.csiAll.symbol, name: MarketTarget.csiAll.name, lastPrice: 4_012, previousClose: 4_000, quoteTimestamp: Date(), provider: "fixture")
            let body = NotificationPolicy.body(for: .baola, target: .csiAll, quote: csiQuote)
            expect(body.contains(MarketTarget.csiAll.name))
